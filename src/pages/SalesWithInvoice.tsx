...
import { ShoppingCart, Plus, Minus, Trash2, Receipt, FileText, X } from "lucide-react";
import { toast } from "sonner";
import { getAllStores, getProductsByStoreId, Store, Product } from "@/data/storeData";
import { useSalesWithStock, SaleItem } from "@/hooks/useSalesWithStock";
import { Badge } from "@/components/ui/badge";

const SalesWithInvoice = () => {
  ...
  const { sales, processSale, generateInvoiceFromSale, cancelSale } = useSalesWithStock();
  ...
  const handleGenerateInvoice = (saleId: number) => {
    const invoiceData = generateInvoiceFromSale(saleId);
    if (invoiceData) {
      // Aqui você pode redirecionar para a página de fatura ou abrir em nova aba
                ...
                {sales.length === 0 ? (
                  ...
                  sales.map((sale) => (
                    <div key={sale.id} className="border p-3 rounded space-y-2">
                      ...
                      {sale.status === 'completed' && (
                        <div className="flex gap-2">
                          ...
                          >
                            ...
                            {sale.invoiceGenerated ? 'Faturado' : 'Gerar Fatura'}
                          </Button>
                          <Button
                            size="sm"
                            variant="destructive"
                      ...
                      {sale.invoiceGenerated && sale.invoiceNumber && (
...
export default SalesWithInvoice;
